...
float **reservaMatriz(int r, int c); 
...
void liberaMatriz(float **m, int r); 
...
int main()
{
    ...
    liberaMatriz(matriz,number); 
    return 0;
}
float **reservaMatriz(int r, int c) {
    float **m; 
    int i;
    //reserva arreglo de apuntadores
    m = (float **) malloc(r * sizeof (float *));
    if (m != NULL) {
        for (i = 0; i < r; i++) {//reserva los renglones
            m[i] = (float *) malloc(c * sizeof (float));
            //verifica que operaci�n exitosa
            if (m[i] == NULL) break;
        }
        //en caso de no reservar todos los renglones
        if (i < r) {
            //libera renglones
            while (i >= 0) free(m[i--]);
            //libera arreglo de apuntadores
            free(m);
            //asigna valor NULL a la matriz
            m = NULL;
        }
    }
    return m;
